  ...
  useEffect(() => {
      ...
      if (interactionMode === 'PICK_POINT') {
          ...
          map.on('mouseup', () => {
              ...
              const radius = circleLayerRef.current.getRadius();
  ...
  useEffect(() => {
    ...
    if (filterPrice !== '全部') {
        const getRange = (label: string) => {
             if (label === '5000元以下') return { min: 0, max: 5000 };
             if (label === '5000-8000元') return { min: 5000, max: 8000 };
             if (label === '8000-15000元') return { min: 8000, max: 15000 };
             if (label === '15000元以上') return { min: 15000, max: Infinity };
             return { min: -1, max: -1 };
        };
              ...
              <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                      <button 
                        ...
                        className={`flex-1 py-1.5 rounded border text-xs font-bold transition-all flex items-center justify-center gap-1 ${
                            interactionMode === 'PICK_POINT' ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-700 border-slate-600 text-slate-300 hover:bg-slate-600'
                        }`}
                      >
                          📍 地图选点
                      </button>
                      <button 
                        ...
                        className={`flex-1 py-1.5 rounded border text-xs font-bold transition-all flex items-center justify-center gap-1 ${
                            interactionMode === 'DRAW_CIRCLE' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-700 border-slate-600 text-slate-300 hover:bg-slate-600'
                        }`}
                      >
                          ⭕ 画圈找房
                      </button>
