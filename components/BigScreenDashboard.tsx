...
const BigScreenDashboard: React.FC<BigScreenDashboardProps> = ({ properties, orders, onExit }) => {
  ...
  useEffect(() => {
    ...
    // 1. Init Map (Center on China)
    const map = L.map(mapContainerRef.current, {
        center: [39.9042, 116.4074], // Center on Beijing for better demo view
        zoom: 11,
        zoomControl: false,
        attributionControl: false,
        dragging: false, // Lock view for dashboard feel
        scrollWheelZoom: false,
        doubleClickZoom: false
    });
    
    // 2. Dark Tech Theme Tiles
    ...
    // Define custom icon class in CSS styles below
    ...
    // 4. Auto Cycle Popups to simulate "Real-time Monitoring"
    let currentIndex = 0;
