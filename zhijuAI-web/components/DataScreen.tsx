...
const DataScreen: React.FC<DataScreenProps> = ({ properties, onViewProperty }) => {
  ...
  return (
    <div className="flex h-full relative overflow-hidden bg-slate-900">
      ...
      {/* Sidebar Control Panel - Optimized (Balanced Size) */}
      <div className="w-80 bg-slate-800/95 border-r border-slate-700 flex flex-col z-10 shadow-xl backdrop-blur-sm">
        ...
        <div className="p-4 flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3 shrink-0">
            <div className="bg-slate-700/50 p-3 rounded-lg border border-slate-600/50 flex flex-col justify-center items-center">
              <div className="text-slate-400 text-xs">匹配房源</div>
              <div className="text-lg font-bold text-white leading-none mt-1">{stats.total} <span className="text-xs font-normal">套</span></div>
            </div>
            <div className="bg-slate-700/50 p-3 rounded-lg border border-slate-600/50 flex flex-col justify-center items-center">
            ...
            <div className="flex flex-col gap-3">
              <div className="relative">
                <input
                  ...
                  placeholder="输入通勤目的地..."
                  className="w-full bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-xs text-white focus:border-indigo-500 outline-none placeholder:text-slate-500"
                />
                {showSuggestions && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-xl z-50 overflow-hidden border border-slate-200 max-h-48 overflow-y-auto">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg border text-xs font-bold transition-all flex items-center justify-center gap-1 ${interactionMode === 'PICK_POINT' ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-700 border-slate-600 text-slate-300 hover:bg-slate-600'}`}
                >
                  📍 地图选点
                </button>
                <button
                  onClick={() => setInteractionMode(interactionMode === 'DRAW_CIRCLE' ? 'VIEW' : 'DRAW_CIRCLE')}
                  className={`flex-1 py-2 rounded-lg border text-xs font-bold transition-all flex items-center justify-center gap-1 ${interactionMode === 'DRAW_CIRCLE' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-700 border-slate-600 text-slate-300 hover:bg-slate-600'}`}
                >
                  ⭕ 画圈找房
                </button>
              </div>
          ...
          {/* 3. Filter Grid (Semi-Compact) */}
          <div className="pt-3 border-t border-slate-700 shrink-0">
            <div className="grid grid-cols-2 gap-2.5">
              {/* Region - Full Width Row */}
              <div className="col-span-2 flex gap-2">
                <select value={filterProvince} onChange={(e) => { setFilterProvince(e.target.value); setFilterCity('全部'); setFilterDistrict('全部'); }} className="flex-1 bg-slate-800 border border-slate-600 text-slate-300 text-xs rounded px-2 py-2 outline-none">
                  <option value="全部">省份</option>
                  {Object.keys(CASCADING_REGIONS).map(p => <option key={p} value={p}>{p}</option>)}
                </select>
                <select value={filterCity} onChange={(e) => setFilterCity(e.target.value)} disabled={filterProvince === '全部'} className="flex-1 bg-slate-800 border border-slate-600 text-slate-300 text-xs rounded px-2 py-2 outline-none">
                ...
                <select value={filterDistrict} onChange={(e) => setFilterDistrict(e.target.value)} disabled={filterCity === '全部'} className="flex-1 bg-slate-800 border border-slate-600 text-slate-300 text-xs rounded px-2 py-2 outline-none">
              ...
              <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="bg-slate-800 border border-slate-600 text-slate-300 text-xs rounded px-2 py-2 outline-none">
                <option value="全部">全部房型</option>
                {['住宅', '城市公寓', '城中村公寓', '别墅', '工厂', '写字楼', '商铺', '其他'].map(c => <option key={c} value={c}>{c}</option>)}
              </select>

              <select value={filterPrice} onChange={(e) => setFilterPrice(e.target.value)} className="bg-slate-800 border border-slate-600 text-slate-300 text-xs rounded px-2 py-2 outline-none">
              ...
              <select value={filterCommute} onChange={(e) => setFilterCommute(e.target.value)} className="bg-slate-800 border border-slate-600 text-slate-300 text-xs rounded px-2 py-2 outline-none">
              ...
              <select value={filterLease} onChange={(e) => setFilterLease(e.target.value)} className="bg-slate-800 border border-slate-600 text-slate-300 text-xs rounded px-2 py-2 outline-none">
          ...
          <div className="shrink-0">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {PRESET_QUESTIONS.map(q => (
                <button
                  ...
                  className="px-2 py-1 bg-slate-700 text-slate-300 rounded text-xs hover:bg-indigo-600 hover:text-white transition-colors border border-slate-600"
                >
                  {q.label}
                </button>
              ))}
            ...
            <textarea
              ...
              placeholder="自定义详细需求..."
              className="w-full bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-xs text-white focus:border-indigo-500 outline-none h-16 resize-none placeholder:text-slate-500"
            />
          </div>
          ...
          {/* Action Buttons */}
          <div className="pt-2 mt-auto shrink-0 pb-4">
            <button
              onClick={handleAISearch}
              disabled={isSearching}
              className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-3 rounded-lg shadow-lg flex items-center justify-center transition-all text-sm"
            >
              {isSearching ? (
                <>
            ...
            <button onClick={handleReset} className="w-full text-slate-500 text-xs hover:text-white underline mt-2 text-center block">
              重置所有条件
            </button>
          </div>
        </div>
